 ...
 * Ultimate needs to come online. ATK/DEF scale gently so higher tiers read as tougher, not
 * unfairly punishing. Easy is 1/1/1 on purpose: it's identical to the pre-tier stage behavior, so
 * every existing Campaign save's Easy-tier progress means exactly what it always did. */
export const TIER_STAT_MULTIPLIERS: Record<DifficultyTier, { hp: number; atk: number; def: number }> = {
  Easy: { hp: 1, atk: 1, def: 1 },
  Medium: { hp: 1.6, atk: 1.15, def: 1.1 },
  Hard: { hp: 2.3, atk: 1.3, def: 1.2 },
  Super: { hp: 3.2, atk: 1.5, def: 1.35 },
};
