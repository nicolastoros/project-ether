...
/**
 * Fixed magenta wash + a single pink-tinted shimmer sweep for Mythic-rarity creature cards —
 * a solid identity color (matching RarityBadge's --color-rarity-mythic) that breathes in
 * opacity instead of cycling hues. Deliberately calmer than LegendaryCardAura below: LR is the
 * one tier that gets the full animated rainbow treatment, so it stays a "wow" moment instead of
 * Mythic diluting it by sharing the same rainbow effect (they used to be nearly
 * indistinguishable in a still screenshot).
...
export function MythicCardAura() {
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden rounded-[inherit]">
      <motion.div
        className="absolute inset-0 bg-rarity-mythic"
        animate={{ opacity: [0.12, 0.24, 0.12] }}
        transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        className="absolute inset-y-0 w-1/3 -skew-x-12 bg-gradient-to-r from-transparent via-pink-200/55 to-transparent"
        initial={{ x: "-140%" }}
        animate={{ x: "340%" }}
